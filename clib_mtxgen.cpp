...
// Dz1=int(2*Ntam*(radiusc-int(segmentheight/2))/dl);
 ...
 else if (pilingstruc == (char*)"conic")
 {

	radiusc=radiusc;
	radiuscore=conicradius1;
	radiusshell=conicradius2;
	radiusc=radiusshell+cs;
	 
	cout<< radiusc << "conicform" << endl;
	 
 Dx1=int(2*Nmaior*(radiusc+conicheight2*0)/dl);
 Dy1=int(2*Nmaior*(radiusc+conicheight2*0)/dl);
// Dz1=int(2*Ntam*(radiusc-int(segmentheight/2))/dl);
 Dz1=int(Ntam*(2*radiusc*0+conicheight2)/dl);


cout << radiuscore << "radiuscore" << endl;
cout << radiusshell << "radiusshell" << endl;
...
pygeneral (xini,xfin,yini,yfin,zini,zfin,Npix,Npiy,Ntam,Dx1,Dy1,Dz1,radiusc,radiuscore,radiusshell,A,corenumber,shellnumber,cnum,dl,x,y,z,base,width,height,base2,width2,radiusx1,radiusy1,radiusz1,radiusx2,radiusy2,radiusz2,radiusxcs,radiusycs,radiuszcs,cs,rcarbonpt,radiuspt,rcarbonpd,radiuspd,distcentro,alloynumber,basex,basey,basez, lenpystruc, heipystruc,pilingstruc,partes,indorigins, radiussurface, surfaceindex, xlength, ylength, zlength, xlength2, ylength2, zlength2,halfsphereradius1,halfsphereradius2, segmentheight, segmentradius1, segmentradius2, pillheight1, pillheight2, pillradius1, pillradius2,conicheight1,conicheight2,conicradius1, conicradius2);
...
//										incoreshell_pill(ii,cnum,radiusc,radiuscore,radiusshell,corenumber,shellnumber,dl,h,x,y,z,index,aa,pillheight1,pillheight2,pillradius1,pillradius2);
										...
										incoreshell_conic(ii,cnum,radiusc,radiuscore,radiusshell,corenumber,shellnumber,dl,h,x,y,z,index,aa, conicheight1, conicheight2, conicradius1, conicradius2);
...
//	z[ii] = z[ii] - (radiusc)/dl;
	...
	x[ii] = x[ii] - (pillheight2/2.0)/dl;


//	cout << radiusc << "radiusc"<< endl;

	
	if( z[ii] <= radiusc/dl and z[ii] >= 0 and kk == 0 )
	{
...
/*
 * 
 * 							HERE IS THE CONIC NP
 * 
 * 
 */ 


int incoreshell_conic(int ii, int cnum,float radiusc,float radiuscore,float radiusshell,int corenumber,int shellnumber,float dl,int h, float x[80000], float y[80000], float z[80000], int& index,int& aa, int conicheight1, int conicheight2, int conicradius1, int conicradius2)
{
...
//	{
	...
	radiuscoredef = radiuscore-cs/2;
	radiusshelldef = radiusshell-cs/2;
	radiuscdef = radiusc-cs/2;
...
//	radiusc = radiusshell+10.0;
	...
	radiuscore = radiuscoredef*(z[ii]/conicheight2);
	
	radiusshell = radiusshelldef*(z[ii]/conicheight2);
	
	radiusc = radiuscdef*(z[ii]/conicheight2);
...
launcher_general(2.0,20.0,30.0,40.0,1,2,10,1,1,1,4,10.0,10.0, 10.0,10.0,10.0,20.0,20.0,20.0,20.0,30.0,30.0,30.0,20.0,20.0,20.0,45.0,25.0,20.0,20.0,30.0,3,40,40,40,20.0,30.0,10,4,10,5,30,20,50,40,30,60,50,60,30,40,50,50,60,50,60,50,60,80,100);
